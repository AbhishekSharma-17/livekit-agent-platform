...
The card's check is: upload the fixture types to a scratch KB, and `POST .../search` returns hits whose chunks carry `meta.heading_path` and `page`. `KbHit` gains `meta` only in V5-04 (`_asks.md` #5 and #6). Until then, step 4 reads the locators of the returned `chunk_id`s from the scratch database.
