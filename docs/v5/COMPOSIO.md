...
- **Several accounts of one app** (labels, a default per app, account-bound tool names, multi-account Tool Router sessions): R-V5-13 in `PLAN-V5.md` §7, packages V5-53/V5-54.
...
- **D-V5-C8 — Voice hygiene at materialisation (research T §2.3.5).** The LKAP tool's `description` is the Composio description's first sentence (editable), parameters are pinned from `input_parameters` with a "Refresh schema" action that diffs, `max_result_chars=1500`, `result_path="data"`, `silent_reply=False`. The tool name is `<toolkit>_<action>` lower-snake (e.g. `googlecalendar_find_free_slots`), unique per workspace. *Amended by R-V5-13 (V5-53):* the default account's tools keep `<toolkit>_<action>`; another account's are `<toolkit>_<action>__<label slug>`; while an app has more than one account every description starts with `(<label>) ` (existing tools keep their names and gain the prefix on the next refresh-schema with `apply`); every `provider` tool carries its connection's `connected_account_id`, kept in step on reconnect, because an unpinned execute runs on the subject's most recently connected account.
