...
| R2-15 | MEDIUM | **Concurrency caps can be exceeded (race).** `connect` counts live sessions, then inserts, with nothing held in between. Load test: a burst of 50 simultaneous connects to an agent capped at `max_concurrent_sessions=5` admitted **14**. The same check-then-insert pattern is in `prepare_outbound_call` (`max_concurrent_outbound`) and in `routers/text_sessions.py`. The per-agent and per-workspace minute buckets still bound the damage. | `routers/connect.py`, `routers/text_sessions.py`, `limits.py::live_session_count`, `telephony/calls.py::prepare_outbound_call`; `scratchpad/loadtest/race.py` | **Fixed (V2-22, R-V2-34).** `limits.reserve_session_slot` / `limits.slot_lock`: a per-process `asyncio.Lock` per agent (and per workspace for the outbound cap) held from the count through the route's `db.commit()`, plus `SELECT … FOR UPDATE` on the agent / workspace row (Postgres). Used by `connect`, `text-sessions` and `POST /v1/calls` (workspace lock outer, agent inner; refusals audited after release). RUNBOOK §5.1: one api process on SQLite. Verified: with the locks stubbed out the burst tests fail (admitted 50/50), with them exactly the cap is admitted. | `api/tests/test_hardening_v2_22.py::test_a_burst_of_50_starts_at_a_cap_of_5_admits_exactly_5[connect\|text-sessions]`, `…test_a_burst_of_20_dials_at_an_outbound_cap_of_3_admits_exactly_3`, `…test_a_burst_of_dials_at_the_agent_session_cap_admits_exactly_the_cap`, `…test_two_sessions_at_the_cap_admit_one_even_across_processes` (the Postgres case: runs in CI's `test-postgres` job with the process lock replaced, so only `FOR UPDATE` separates the two sessions; not run locally, no Postgres on the dev host) |
| R2-16 | MEDIUM | **One platform-wide service token.** `/internal/v1/*` accepts a single `LKAP_SERVICE_TOKEN` for every workspace: `sessions/{id}/resolved` returns decrypted credentials for any workspace, and `connections/{id}/worker-env` returns any connection's LiveKit secret. The Cloud deploy bundle tells whoever deploys a workspace's worker to paste "the api's service token". The api never returns the token itself; bundles use a placeholder. | `deps.require_service`; `connections/bundle.py:206-218` | **Ruled R-V2-27: deferred with triggers.** Interim done (V2-22): `deploy/Caddyfile` answers 403 on `/internal/*` unless the peer is in `LKAP_INTERNAL_ALLOWED_CIDRS` (default `private_ranges`; wired through `docker-compose.prod.yml` and `prod.env.example`); RUNBOOK §5.1 documents the variable, the cloud-hosted case and that the token is operator-only. | Caddyfile: `caddy validate` not run (caddy not installed on the dev host, Docker not running); reviewed by hand against the Caddy source and docs (V2-22 report). Add `caddy validate` to CI once the remote exists. |
| R2-17 | MEDIUM | **SIP transfer to an arbitrary host.** Per R-V2-23's text, `sip:+15551230000@<any host>` passes the policy by its numeric prefix, whatever the host, so a builder-configured target (or a console transfer) can REFER a caller to an attacker's SIP server. That means call hijack or eavesdropping, not toll fraud. The model can only pick configured targets. | `telephony/policy.py::_number_and_host` | **Fixed (V2-22, R-V2-28).** `_number_and_host` returns both parts for `sip(s):+E164@host`; `destination_problem` needs the prefix rules **and** `host ∈ allowed_sip_hosts`, and names `allowed_sip_hosts` in the message. Save-time validation inherits it. Console hint updated. | `test_telephony_policy.py::test_numeric_sip_user_on_an_unlisted_host_is_422_naming_allowed_sip_hosts`, `…on_a_listed_host_under_plus_one_passes`, `…test_premium_number_on_a_listed_sip_host_is_still_always_blocked`, `…test_transfer_target_with_a_numeric_sip_user_on_an_unlisted_host_is_422_at_save`; `test_telephony.py::test_console_transfer_policy_sip_host_and_numeric_user` (inverted: `by_prefix` is 422) and `test_destination_problem_prefix_and_sip_host_rules` |
| R2-18 | MEDIUM | **Two gaps in the `+1` premium ranges.** (a) `allowed_prefixes=["+1"]` also admits about 20 Caribbean NANP countries (`+1 809/829/849` Dominican Republic, `+1 876` Jamaica, `+1 284`, `+1 473`, `+1 649` …), the classic "one-ring" IRSF destinations. (b) `BLOCKED_PREFIXES` has `+1976`, but NANP 976 pay-per-call numbers are an *exchange* (`+1 NPA 976 xxxx`, e.g. `+1 212 976 …`), which a prefix list can't express, so they pass. | `telephony/policy.py::BLOCKED_PREFIXES` | **Fixed (V2-22, R-V2-29).** `+1` alone admits US and Canada only: `NANP_NON_US_CA_NPAS` (27 NPAs: Caribbean and Atlantic countries and US territories) is excluded unless a longer explicit prefix covers the number; the `^\+1[2-9]\d{2}976\d{4}$` exchange is always blocked (`+1976` kept with a comment). Source: the NANPA table, taken on 2026-09-24 from two mirrors because nationalnanpa.com did not resolve from the dev host (comment in `policy.py`). Console warns while `+1` is the only NANP prefix. | `test_telephony_policy.py::test_plus_one_alone_does_not_reach_the_caribbean_or_the_territories[*]` (8 numbers), `…test_plus_one_still_reaches_the_united_states`, `…test_an_explicit_caribbean_prefix_overrides_the_plus_one_exclusion`, `…test_the_976_exchange_is_always_blocked[*]`, `…test_the_non_us_ca_npa_set_is_the_nanp_table_and_excludes_the_mainland`; `web/tests/console-telephony.test.tsx` "warns that +1 alone covers only the US and Canada" |
| R2-19 | MEDIUM | **`add_member` trusts an unverified email.** Accounts are global, and nothing proves an email's owner. An admin of A can pre-create `alice@b-corp` (invite and accept with a chosen password); if B later uses `add_member` for that email, A's admin holds a B membership. Residual of R2-05. | `routers/workspaces.py::add_member` | **Fixed as ruled (V2-22, R-V2-30).** `add_member` re-adds only an account with a `member.join` / `member.add` / `member.remove` audit row in this workspace; any other existing account → 409 `use_invite`. The Team tab never calls the member route: "Send invite" is its only add path. The residual (accounts carry no verified email) stays a multi-tenant precondition (§8). | `api/tests/test_hardening_v2_22.py::test_add_member_only_re_adds_and_another_workspace_must_invite`, `…test_add_member_of_an_account_with_no_history_here_is_409_use_invite`; `web/tests/console-settings-team.test.tsx` "adds people only through Send invite…" |
| R2-20 | MEDIUM | **`recording.ready` can fire twice with different event ids.** Both the `egress_ended` webhook and the worker's `/recording` fallback schedule the finalise job unconditionally. | `recordings/finalize.py:106`, `routers/internal.py:521`, `recordings/job.py:46-63` | **Fixed (V2-22, R-V2-36).** `recordings.finalize.schedule_finalize_once` schedules the one `recording_finalize` job only while the recording is `ready` and only when none exists for the session; used by the `egress_ended` handler and the worker fallback (`routers/internal.py`). "Schedule on the transition" was not usable: V2-03's built-in handler sets `ready` first. | `api/tests/test_hardening_v2_22.py::test_egress_ended_and_the_worker_fallback_deliver_recording_ready_once[webhook_first\|worker_first\|worker_twice]`, `…test_a_not_yet_ready_report_does_not_use_up_the_one_finalise_job` |
| R2-21 | MEDIUM | **Supply-chain pins are tags, not digests.** GitHub Actions are pinned by major tag (`actions/checkout@v4` …), base images by tag (`python:3.12-slim-bookworm`, `node:24-alpine`, `ghcr.io/astral-sh/uv:0.12.15`, compose `postgres:16` …). The agent image installs flavour requirements (`requirements/{slim,full}.txt`) outside `uv.lock` with no hashes. Lockfiles themselves are used frozen everywhere (`uv sync --frozen`, `pnpm install --frozen-lockfile`). | `.github/workflows/*`, `*/Dockerfile`, `agent/Dockerfile:103-111` | **Open (documented).** Pin actions by SHA (Dependabot keeps them current), pin images `@sha256:`, and compile flavour requirements with `--generate-hashes` then install with `--require-hashes`. This needs the GitHub remote the user hasn't created yet. | n/a |
...
| R2-26 | LOW | **KB uploads put the client filename into the object key.** `LocalStorage._path` blocks escape and S3 treats `..` literally. | `routers/knowledge.py:316,327` | **Fixed (V2-22).** `routers/knowledge.py::upload_basename` keeps the last path segment (backslashes too); blank or dot-only names become `document`. | `api/tests/test_hardening_v2_22.py::test_kb_upload_keys_use_the_basename[*]` |
...
| R2-34 | LOW | **S3 `endpoint_url` isn't guarded.** An admin-set `storage_configs.endpoint_url` reaches botocore without the net guard (signed-URL generation needs no request; Egress uploads run on LiveKit's side). The KB embedder isn't affected: `resolve_embedder` builds `OpenAIEmbedder` with only the credential's `api_key`, so its base URL stays the fixed OpenAI one. | `storage/s3.py`, `kb/embed.py:235` | **Guard added (V2-22).** No route writes `storage_configs.endpoint_url` yet; `storage/endpoint.py::validate_endpoint_url` (422 `blocked_destination`, `LKAP_NET_ALLOW_PRIVATE_HOSTS` applies) is the check any future write route must call. The operator's `LKAP_STORAGE_ENDPOINT_URL` (private `minio:9000` in the prod compose) is deliberately not checked. | `api/tests/test_hardening_v2_22.py::test_an_admin_set_storage_endpoint_on_a_private_address_is_422[*]`, `…test_public_or_absent_storage_endpoints_pass[*]` |
...
| R2-36 | LOW | **Other secrets on command lines.** The prod compose `mc alias set … "$$MINIO_ROOT_PASSWORD"` puts the MinIO password in argv. | `deploy/docker-compose.prod.yml:269` | **Fixed (V2-22).** `minio-init` gets the alias from `MC_HOST_local` (compose-time URL with the keys; `prod.env.example` asks for URL-safe values); `mc alias set` and its password argument are gone. | YAML parses; compose not run (no Docker daemon) |
...
| R2-38 | MEDIUM | **aiohttp follows a redirect to an IP literal around the guard** (found at sign-off). aiohttp's `TCPConnector._resolve_host` returns IP-literal hosts without consulting the resolver, so `GuardedResolver` never sees them, and the LiveKit `TwirpClient` posts with the default `allow_redirects=True`. Reproduced on the tree: a `307` from a host standing in for an admin-controlled public server sent the Twirp POST to `http://127.0.0.1:<port>/latest/meta-data/`, which answered 200 with its body; the resolver was never called. From the api's network position this relays a POST body (307/308) to any internal address, metadata included, and feeds the response to the Twirp parser and the probe message. The `Authorization` header did not follow the redirect (aiohttp drops it on a host change), so the connection's JWT does not leak. Needs an admin to set a connection URL. Partially reopens R2-08 until fixed. | `net_guard.guarded_aiohttp_session`, `connections/clients.py::api`; aiohttp 3.14.3 `connector.py::_resolve_host`; `livekit.api.twirp_client` (1.2.1) | **Fixed (V2-22, R-V2-26).** `net_guard.GuardedTCPConnector` runs IP-literal hosts through `address_problem` in `_resolve_host`; `net_guard.NoRedirectClientSession` forces `allow_redirects=False` on every request and turns a 3xx answer into `BlockedDestinationError` (so the probe says `blocked destination`); both wired in `guarded_aiohttp_session`, which `ConnectionClientFactory.api` uses. R2-08 is closed again. | `api/tests/test_hardening_v2_22.py::test_guarded_session_never_follows_a_redirect_to_a_loopback_literal`, `…test_probe_through_a_redirecting_url_says_blocked_and_never_hits_the_target`, `…test_guarded_connector_refuses_a_redirect_it_is_asked_to_follow`, `…test_guarded_connector_refuses_inward_literals_under_the_prod_policy[*]`, `…test_guarded_connector_allows_a_literal_the_policy_exempts_but_never_metadata`, `…test_the_livekit_session_is_the_guarded_one` |
| R2-39 | LOW | **Non-canonical numeric hosts pass the offline check** (found at sign-off). `check_url` (api) and `check_url_allowed` (worker) treat `2130706433`, `0x7f000001`, `127.1` and `0` as names, so a webhook or connection URL in that form saves with 201. The connect-time guard catches them on both sides (`getaddrinfo` returns `127.0.0.1` / `0.0.0.0`, which the resolved-address check refuses; verified), so this is defence in depth. `0177.0.0.1` is rejected by httpx outright. | `net_guard.host_problem`, `_http_safety.is_private_host` | **Fixed (V2-22, R-V2-26).** `net_guard.numeric_host` / `_http_safety.is_numeric_host`: a host that is not a canonical IP literal but whose last label is all digits or a `0x` number is refused offline on both sides (a last label, so `0xabc.example.com` still passes). | `api/tests/test_hardening_v2_22.py::test_numeric_webhook_url_is_422_blocked_destination_at_save[*]`, `…test_numeric_connection_url_is_422_blocked_destination_at_save[*]`, `…test_numeric_hosts_are_refused_offline[*]`; `agent/tests/unit/test_http_safety.py::test_numeric_hosts_are_refused_before_any_resolution[*]` |
| R2-40 | LOW | **The web proxy's CSRF guard keys only on `Sec-Fetch-Site`.** A browser that doesn't send it (Safari before 16.4) with the dev break-glass token on can be made to POST to `/api/console/*` cross-site. Dev only: in production the bypass is off and the api's own `Origin` check guards cookie writes. | `web/src/app/api/console/[...path]/route.ts::crossSiteWrite` | **Fixed (V2-22, R-V2-36).** While the break-glass bypass is on, the proxy also refuses a write whose `Origin` is present and is not the request's own origin (including `null`). With the bypass off the api's own Origin check applies, as before. | `web/tests/console-proxy-auth.test.ts` "refuses a write with a foreign Origin and no Sec-Fetch-Site (R2-40)", "refuses an opaque (null) Origin write too", "forwards a write whose Origin is its own…", "leaves the Origin check to the api once the bypass is off" |
