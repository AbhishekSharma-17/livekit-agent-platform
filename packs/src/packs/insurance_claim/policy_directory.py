"""Mock policy directory used by the insurance pack's background policy lookup.

Ported verbatim (behaviour-for-behaviour) from the original ``policy_directory.py``
(commit ``c4472b0``). This stands in for a carrier policy administration system.
The records line up with the demo prompts in :mod:`packs.insurance_claim.scenarios`
(``packs/tests/insurance_claim/scenarios.py``) so a live call can verify the
policy the claimant mentions while the conversation keeps going.
"""
...
POLICY_RECORDS: dict[str, dict[str, Any]] = {
    ...
    "AUTO90210": {
        ...
        "notes": [
            "Two listed drivers.",
            "Rental reimbursement is available while the vehicle is in the shop.",
        ],
    },
    "RNT3008": {
...
def lookup_policy(policy_number: str) -> dict[str, Any]:
    ...
    if not key:
        return {
            ...
            "policy_number": "",
            "message": (
                "No policy number was provided. Ask the claimant to read it from their ID "
