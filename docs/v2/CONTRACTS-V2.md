...
- `BlockSpec{id, type: BlockType, title: str|None, config: dict, order: int}`; `BlockType = Literal["status","notes","checklist","activity","form","document","gallery","table","transcript","video","kb_citations","custom","choices","details","markdown","steps"]` (the last four: V5-08, below).
- Block state models (exported to TS, validated by the reducer): `FormBlockState{schema: dict (JSON schema), values: dict, status: idle|requested|submitted|cancelled, submitted_at}` (a `RequestableState`, V5-02), `DocumentBlockState{asset_id|url, page:int, highlights:[{page, bbox, note}]}`, `GalleryBlockState{asset_ids: list[str], selected: str|None}`, `TableBlockState{columns:[{key,label,type}], rows: list[dict], selected_row: str|None}`, `TranscriptBlockState{show_tools: bool}`, `VideoBlockState{source: agent_avatar|user_camera|user_screen|track:<sid>, muted}`, `KbCitationsBlockState{items:[{chunk_id, filename, score, text}]}`; `status/notes/checklist/activity` blocks render the envelope fields and hold `{}`.
...
- **Generic blocking request (V5-02; `docs/research-v4/panels-and-capabilities.md` §3.0 item 1).** Every block that asks the user for an answer shares one round trip instead of a per-type RPC:
  - State: `RequestableState{status: idle|requested|submitted|cancelled, submitted_at: float|None}` is the mixin of every requestable block's state model; `FormBlockState` inherits it (its status set gains `cancelled`; v2 form JSON still validates). `status == "requested"` is the pending marker: a reconnecting browser renders the pending request from the snapshot alone, as `form` always did.
  - `UiRequest.method` += `"request"`, payload `BlockRequestPayload{block_id, timeout_s, schema?, …block-specific keys}`. The browser acks at once (`{}`); an inline `{values}` or `{cancelled: true}` result is accepted too.
  - `AgentAction.action` += `"block_submit"`, payload `BlockSubmitPayload` = `{block_id, values: dict}` or `{block_id, cancelled: true}` (exactly one). Accepted for any requestable block (a `RequestableState` type, `custom`, or an untyped block) and for any block with a pending request, so it also answers a `form`.
  - Worker: `UiChannel.request_block(block_id, *, timeout_s, payload=None) -> dict | None`. The caller writes the block's content first; `request_block` patches only `status = "requested"` and `submitted_at = null`, sends the RPC, and awaits a future keyed by block id (one pending request per block). It returns the values on submit (stored at `values` when the state model has that field), and `None` on a user cancel, a timeout or `cancel_pending` (status → `cancelled`, session event `block_update {op: "block_cancelled", reason?}`), on a second request for the same block (the newer request owns the status), and on session close. `UiChannel.cancel_pending(reason, *, methods=None) -> list[str]` is synchronous and releases every pending request (the session calls it with `reason="barge_in"` when the caller speaks while a request is pending; `methods=["request"]` spares legacy forms); `UiChannel.pending_requests` lists what is waiting. A submission nobody awaits reaches the unsolicited-form callback.
  - Realtime and half-cascade pipelines: the requesting tool returns `None` at once and runs the wait on the session's `BackgroundRunner`; a submission arrives as an urgent background result, a missed request as a routine note (the `request_form` behaviour; the tool's job, not the channel's).
  - **`form` is kept as an alias for one release** (deprecated, no runtime warning): `request_form` runs on the same machinery with `method="form"`, payload `{block_id, schema, prefill}` and its v2 statuses (cancel → `idle`, timeout keeps `requested` so a late submission still lands); `form_submit` stays valid for `form` blocks beside `block_submit`.
- **The block quartet (V5-08; research P §3.2 B1–B4, E1; D-V5-33, D-V5-34, R-V5-1).** Four block types, each with a strict config (`contracts/blocks.py`) and an exported state model:
  - `choices` — config `{multi: bool=false, layout: buttons|list|chips="buttons", max_options: 2..20=8}`; state `ChoicesBlockState(RequestableState){prompt, options: [{id, label, hint?, tone?, image_asset_id?}], multi, selected: list[str], reveal: {correct, explanation?}|None}`. Tools: `request_choice(prompt, options: [{id, label, hint?}], multi=false, block_id?)` writes the question and options, then `request_block` (60 s): cascaded returns `{"selected": [...], "labels": [...]}` or a "not picked" line; realtime/half-cascade returns `None`, runs the wait in the background (a tap is an urgent result, a miss a routine note) and its reply is silenced (`_REALTIME_SILENT_BUILTINS`; not on phone or text channels, where the tool answers at once); `sip_in`/`sip_out` answer `{"channel": "voice_only"}` and show nothing; `text` answers at once. `resolve_choice(selected, block_id?)` records a spoken answer through `UiChannel.submit_block(block_id, values)` (resolves a pending request; `via: "voice"` keeps the background waiter quiet; with nothing pending the block is just marked submitted, no late-submit prompt). A browser answers with `block_submit {values: {selected: [...]}}`; the channel writes `selected` when the ids are options of the block (one unless `multi`). **Barge-in**: `PlatformAgent` registers a synchronous `user_state_changed` handler once per session and calls `cancel_pending("barge_in", methods=["request"])` on `new_state == "speaking"`; a pending `form` is untouched (R-V5-1).
  - `details` — config `{columns: 1..2=1, fields: [{key, label, type}]}` (the fields seed `items` with no value); state `DetailsBlockState{items: [{key, label, value: str|number|null, type: string|number|date|money|phone|email|badge, tone?, updated_at?}]}`. Tool `set_details(items: [{key, value: str, label?}], block_id?)` upserts by `key` (a `number`/`money` row stores a number when the text reads as one; `""` clears).
  - `markdown` — config `{max_chars: 200..50000=8000, allow_links: false}`; state `MarkdownBlockState{markdown, title?, updated_at?}`, rendered as a strict Markdown subset (no raw HTML, links only when allowed, images only from session assets). Tool `show_text(markdown, title?, block_id?)` replaces the text; raw HTML or text over `max_chars` is a `ToolError`; phone channels answer `{"visible": false}`.
  - `steps` — config `{steps: [{id, label}], source: flow|manual="manual", show_notes: true}`; state `StepsBlockState{steps: [{id, label, status: pending|active|done|skipped|failed, note?, at?}], current}`. Tool `set_steps(steps: [{id, status, label?, note?}], current?, block_id?)` only for `source="manual"` blocks. With `source="flow"` the flow runtime writes it on every step change (the configured steps, or the flow's agent nodes: visited → `done`, current → `active`, passed over → `skipped`; after an end node nothing is current). A `custom` block with `kind == "flow_progress"` keeps its raw mirror (§4.5); the console renders it with the `steps` renderer (D-V5-33).
  - `BLOCK_TOOL_NAMES` += `request_choice`, `resolve_choice`, `set_details`, `show_text`, `set_steps` (all in `NEVER_BACKGROUND_TOOLS`); `UPDATABLE_BLOCK_TYPES` += `details`, `markdown`, `steps`.
  - **Citations (E1)**: `KbCitation` += `document_id?, page?, heading_path?, char_start?, char_end?` (from the chunk's ingest locators; unset ones stay off the wire). Tapping one sends `block_action {block_id, name: "open_citation", data: {chunk_id} | {index}}`, handled by the platform (never the pack): when the session holds the document as an asset (`AssetRef.meta.document_id`, V5-19) and the panel has a `document` block, that block opens on the cited page with the heading path as its note and the result is `{opened: "document", block_id, page}`; otherwise `{opened: false, reason: unknown_citation|no_document_block|no_source, citation?}` so the browser can preview the passage itself.
  - Validation: the configs are checked on save like every block; a `source="flow"` steps block warns when the agent has no flow or names a step the flow does not have; a `choices` block warns on an agent set up for phone calls (`capabilities.dtmf` or `telephony.transfer_targets`), since phone callers see no screen.
  - The late-submit (unsolicited) prompt is block-type neutral: "The user just submitted the {block_id} block on screen with these values: …" (R-V5-1).
