...
| Console constants | `@/components/console/lib/constants` | `TONE_BADGE_CLASSES` (token classes), `PANEL_META: Record<string, {label, description, layout: "side"\|"wide"}>`, `CAPABILITY_META: Record<"camera"\|"screen_share"\|"chat_input"\|"vision_inject_per_turn", {label, description, iconName, icon}>`. Console-only — never import from session code. |
