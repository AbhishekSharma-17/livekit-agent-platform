...
**Chain.** `down_revision = "v4_002_provider_models"`. §0.3 chains V5 after the costs package's `v4_003`, which has not landed; if it lands first, the coordinator re-chains `v5_001` onto it (`_asks.md` #1) and re-runs this rehearsal.
...
`alembic/env.py` gains `include_object`, so autogenerate skips the FTS table, its shadow tables, `kb_chunks.tsv` and `ix_kb_chunks_tsv` (`_asks.md` #2).
