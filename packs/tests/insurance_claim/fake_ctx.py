"""`FakePackSessionContext` -- a pack-local, in-memory `packs.base` implementation
for offline pack/tool unit tests (no LiveKit connection, no vendor keys, no
network).

This is a deliberately separate copy of `agent/tests/fakes/fake_ctx.py`
(W0-SCAFFOLD), not an import of it: `packs` cannot depend on `lkap_agent`
(docs/ARCHITECTURE.md §10, "packs never import `lkap_agent`") and `packs`'
own dev dependencies don't include `structlog` or the agent test package's
`fakes.fake_room`, both of which the agent-side fake pulls in. See the work
package report's "deviations" section.
"""
