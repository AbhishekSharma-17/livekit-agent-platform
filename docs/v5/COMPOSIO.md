...
- `routers/tools.py::_check_payload`: a `provider` definition may bind `credential_id` → a `composio` credential and `connection_id` → a `tool-provider-account`; an `mcp` definition with `origin.provider == "composio"` may bind the `composio` credential for its `x-api-key` header (this is the only `_check_payload` change V5-18/47 make; V5-09 generalises header auth later and must keep this case).
