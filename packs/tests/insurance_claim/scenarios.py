"""Demo claimant narratives, ported verbatim from the original ``examples.py`` (commit ``c4472b0``).

Used by ``test_workflow.py`` and reusable by live tests / a console "sample
claims" hint (docs/INSURANCE_PACK_MAPPING.md #28).
"""
...
DEMO_PROMPTS: dict[str, str] = {
    "basement_flood_with_photos": BASEMENT_FLOOD_WITH_PHOTOS.strip(),
    "car_accident_with_injuries": CAR_ACCIDENT_WITH_INJURIES.strip(),
    "stolen_laptop_no_police_report": STOLEN_LAPTOP_NO_POLICE_REPORT.strip(),
    "travel_cancellation_storm": TRAVEL_CANCELLATION_STORM.strip(),
    "incomplete_vague_claim": INCOMPLETE_VAGUE_CLAIM.strip(),
}

__all__ = ["DEMO_PROMPTS"]
