...
| Tool | Input | Output | Scope |
|---|---|---|---|
| `chat_start` | `agent_id_or_slug`, `participant_name="lkap-mcp"`, `wait_for_greeting=true`, `timeout_s=30` | `Chat{chat_id, session_id, agent{id,slug,name,mode,pipeline_mode}, greeting: Untrusted?, cost_hint}`. **Worker preflight:** the tool first reads `GET /v1/connections/{agent.connection_id}/fleet` and returns `ok=false, code="no_worker"` with `next_steps` (the worker-env template for `external`, or `connection_fleet(action="start")` for `supervised`) when no instance is `ready`, so a session is never minted against an empty pool | `agents:write` (text sessions on drafts need a privileged caller; an API key is one) |
| `chat_send` | `chat_id`, `text`, `timeout_s=60` | `{replies: Untrusted[], turn_index, events: SessionEventOut[] (tool calls, block updates since the last turn), state}` | `agents:write` |
| `chat_rewind` | `chat_id`, `turn_index`, `replace_text?` | the regenerated reply (the `rewind` / `inject_user_text` actions of V2-18) | `agents:write` |
| `chat_end` | `chat_id` | `{session_id, turns, url: the session detail path}`; transcript and QA arrive on `session_get` after the worker's summary | `agents:write` |

Scope note: `POST /v1/agents/{id}/text-sessions` is a public-connect route with an optional principal and no scope check of its own (an API key counts as privileged through `member_role_in`), so the `agents:write` gate above is the MCP's, stricter than the api's; tightening the route to require `agents:write` for API keys is a Phase 2 api ask, not a v3 change.
...
1. **Section "Connect an AI agent"** with one primary button that opens the **Connect dialog** (a `Dialog`, ≤ 640 px, three steps in one dialog body):
   ...
   - *Step 2 — Reveal.* The raw key once with `CopyButton` and the same "shown once" copy as the API keys tab; the setup snippets in `Tabs` per client, each with a `CopyButton`, generated for the chosen connection kind:
     ...
     - Remote · Codex CLI: `[mcp_servers.lkap]` with `url = "<LKAP_MCP_PUBLIC_URL>"` and the bearer header in the form the installed Codex version documents (`http_headers` map, or `bearer_token_env_var` with the key exported in the shell); V3-04 verifies the exact key names against the Codex config reference at build time and the snippet test pins them.
...
| MCP sessions per key | 5 (`LKAP_MCP_MAX_SESSIONS_PER_KEY`) | service; the 6th `initialize` → `429` |
| Tool calls per session | 120/min (`LKAP_MCP_CALLS_PER_MIN`) | service; a token bucket per session, `429` with `retry_after_s` |
...
The connect dialog's "Remote (HTTP)" choice (§5) emits, for the chosen client: Claude Code `claude mcp add -s user --transport http lkap <LKAP_MCP_PUBLIC_URL> --header "Authorization: Bearer <key>"`; Codex `[mcp_servers.lkap]` with `url` and the bearer header form the installed Codex documents (verified by V3-04 at build time); generic `mcpServers` JSON with `url` and `headers`. The same "user-level config, not a committed file" line follows each.
...
5. **Resource limits:** the §9.3 table, plus a 60 s per-tool-call timeout and a cap of 10 in-flight calls per session; oversized body → `413`.
