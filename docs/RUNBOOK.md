...
| `LKAP_MCP_ALLOWED_HOSTS` | optional | optional | | | comma list of the hosts MCP servers may live on (V5-09). Empty = any public `https` host that passes the network guard; a list is a ceiling, checked when a tool is saved or tested (api) and when a session connects (worker); `@http` reuses `LKAP_HTTP_TOOL_ALLOWED_HOSTS`, and then an empty list allows no MCP server. Set the same value on both, and include the Apps provider's host when agents use Apps servers. |
...
- **Self-hosted LiveKit on a private network or tailnet (R-V2-26).** A connection marked **self-hosted** is an admin-configured destination: its url may resolve to a loopback, RFC 1918 private, IPv6 ULA (`fc00::/7`) or CGNAT (`100.64.0.0/10`) address, so a LAN server (`ws://10.20.0.5:7880`), `ws://localhost:7880` in `prod`, or a Tailscale node (`wss://livekit.example-tailnet.ts.net`, which resolves to a `100.x` address) works with no allowlist entry. Cloud metadata (`169.254.169.254`, `100.100.100.200`, `fd00:ec2::254`), link-local, multicast and reserved addresses stay refused, names are still resolved and checked at connect time, and redirects are never followed. A connection marked **Cloud** keeps the public-only rule; its refusal says to mark the connection self-hosted if the server really is on your network. Everything else the api calls (webhook endpoints, HTTP-tool dry runs, QA judges, MCP servers) stays public-only unless its host name or CIDR is in `LKAP_NET_ALLOW_PRIVATE_HOSTS`, e.g. `LKAP_NET_ALLOW_PRIVATE_HOSTS=hooks.internal,10.20.0.0/16`; metadata addresses stay refused whatever is listed, so list only what the api must reach. A future storage-config route must check S3 `endpoint_url`s the same way (`storage/endpoint.py`); the operator's own `LKAP_STORAGE_ENDPOINT_URL` is not checked.
...
- **The rule.** An id is 1–200 printable ASCII characters with no whitespace, no `? # & = < > " ' `` ` ``, no `://`, and it does not start with `http`. A value that looks like an API key (a known key prefix such as `sk-`, `AIza`, `xai-`, or 32+ characters of one class with no `/ . : -`) is refused first. Both are validation **errors**, and no message ever repeats the value. The same rule covers `type="model"`/`type="catalog"` fields and the id fields (`voice`, `voice_id`, `avatar_id`, `face_id`, `pal_id`, `persona_id`, `voice_name`, `emotion_id`), except that on a catalog or id field a bare token without a key prefix is only a **warning** (some vendors' real ids are 32 hex characters; R-V4-31). `GET /v1/providers` returns the rule as `model_id_rules`.
- **Access logs.** The model id is part of the URL of `GET`/`PUT /v1/providers/{id}/models/{model_id}`, so the HTTP server's (and any reverse proxy's) access log records it like any request line; the platform's own structured logs (`lkap_api`, `lkap_agent`, `lkap_mcp`) never do, and an unhandled error logs the route template, not the path. The two first-party clients check the id before they send it — the console's model field and the MCP tools `provider_test_model`, `provider_model_declare` and `lkap_describe("model", …)` refuse a key-shaped value locally — so a key lands in an access log only through a hand-written request (R-V4-32).
