...
4. **Knowledge seeds.** `kb/seed.py` is generalised: `import_kb_seeds(db, store, embedder, *, root: Traversable, source_label, seeds, workspace_id)` reads files under `root/"seeds"`; `import_pack_kb_seeds` becomes a thin wrapper that resolves the pack's `importlib.resources.files(module_path)`; templates pass their catalogue directory. Reuse-by-name semantics are unchanged, which is why every template KB name is **prefixed with the template name** ("Knowledge assistant · Product FAQ") so two starters never merge into one KB. Since V4-18 (R-V4-65) seeds take the upload path — `pending` documents with their bytes in storage, one `KB_INGEST` job each after the agent row commits — so the create returns at once and seeded knowledge is ready when its documents are, usually within seconds (`kb_get` shows the status).
