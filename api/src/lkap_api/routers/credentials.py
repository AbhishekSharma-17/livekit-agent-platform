...
async def _load(db: AsyncSession, credential_id: str) -> Credential:
...
async def create_credential(
    payload: CredentialCreate, db: DbDep, vault: VaultDep, _admin: AdminDep
) -> CredentialOut:
...
async def get_credential(credential_id: str, db: DbDep, _admin: AdminDep) -> CredentialOut:
    """Return one credential's metadata."""
    return _to_out(await _load(db, credential_id))
...
async def update_credential(
    ...
    row = await _load(db, credential_id)
...
async def _references(db: AsyncSession, credential_id: str) -> list[str]:
    """Return human-readable references to a credential (agents and tools)."""
    found: list[str] = []
...
async def delete_credential(credential_id: str, db: DbDep, _admin: AdminDep) -> Response:
    """Delete a credential that nothing references."""
    row = await _load(db, credential_id)
    refs = await _references(db, credential_id)
    if refs:
...
@router.post(
    ...
    description=(
        ...
        "check (avatars, tool secret bags) report `ok=true` with a note."
    ),
)
...
async def seed_bootstrap_credentials(db: Any, vault: Vault, raw_json: str | None) -> int:
    ...
    are idempotent and never overwrite an admin's key.
    ...
    for provider_id, secrets in parsed.items():
        try:
            ...
            continue
        existing = (
            await db.execute(select(Credential.id).where(Credential.provider_id == provider_id))
        ).first()
