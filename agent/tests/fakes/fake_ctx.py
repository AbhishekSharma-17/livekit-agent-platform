"""`FakePackSessionContext` — an in-memory implementation of every
`packs.base` Protocol, for offline pack/tool unit tests (no LiveKit
connection, no vendor keys, no network).

Used by W1-AGENT-UI, W1-AGENT-TOOLS, W1-PACK-INSURANCE-CORE and
W2-PACK-INSURANCE-TOOLS. Each fake below stores its calls/results in plain
lists/dicts so a test can assert on them directly (e.g.
`ctx.ui.state.notes`, `ctx.workflow_llm.calls`, `ctx.background.jobs`).
"""
...
from lkap_contracts.agent_config import PipelineMode as _PipelineMode
...
def default_agent_config(**overrides: Any) -> AgentConfig:
    """A minimal, valid `AgentConfig` (cascaded LiveKit Inference), for tests
    that don't care about pipeline details.
    """
...
class FakeImageGen:
    """In-memory `packs.base.ImageGen`. Tests configure `.image_bytes`/`.mime`."""
...
class FakeBackgroundRunner:
    ...
    `await asyncio.sleep(0)` (or await `wait_idle()`) to let it settle, then
    assert on `.urgent_events` / `.routine_notes` / `.on_result_calls`.
    """
...
class _FakeAgentSession:
    ...
    pack/tool code commonly makes. Wave-1 suites that need real `AgentSession`
    behaviour (metrics, realtime events, ...) bring their own richer fake
    (`agent/tests/fakes/fake_llm.py` et al., W1-AGENT-CORE) and pass it to
    `FakePackSessionContext(session=...)` instead.
    """
...
#: Distinguishes "caller omitted `image_gen`" (default to a `FakeImageGen()`) from
#: "caller explicitly passed `image_gen=None`" (no image-gen credential configured,
#: matching `PackSessionContext.image_gen: ImageGen | None`).
_UNSET: Any = object()
...
class FakePackSessionContext:
    ...
    def __init__(
        ...
        *,
        ...
        pipeline_mode: _PipelineMode = "cascaded",
        ...
        self.room = room or cast(rtc.Room, _empty_fake_room())
        ...
        self.log = structlog.get_logger("test").bind(session_id=session_id, agent_id=agent_id)
