...
Ported behaviour-for-behaviour from the original ``policies.py`` (commit
``c4472b0`` of the standalone demo, extracted directly from git history since
it has since been deleted from the working tree): identical rule logic,
including three documented bugs kept for exact parity (see
``packs/tests/insurance_claim/test_rules.py`` xfail cases, ported from the
original ``tests/test_policies.py``). Only the import path and
``Optional[X]`` -> ``X | None`` changed.
"""
...
#: The packet's fixed coverage disclaimer paragraph, pulled out to a constant only so its
#: source line fits the line-length limit; the rendered Markdown is unchanged.
_COVERAGE_DISCLAIMER = (
    "This is an intake triage packet for demo and educational use. It does not confirm "
    "coverage, benefits, liability, payment, or legal rights. A licensed adjuster or "
    "qualified human reviewer must evaluate the applicable policy, endorsements, exclusions, "
    "deductibles, documentation, and governing law."
)


def _as_model[ModelT: BaseModel](model_type: type[ModelT], value: Any) -> ModelT:
...
def _without_negated_safety_mentions(text: str) -> str:
    ...
    negated_patterns = [
        ...
        r"(?:injur\w*|hurts?|pain|medical attention|ambulance|hospital|unsafe|hazard\w*|danger)\b",
        r"\b(?:injur\w*|hurts?|pain|medical attention|ambulance|hospital|unsafe|hazard\w*|danger)"
...
def _has_positive_safety_language(text: str) -> bool:
    ...
    return _has_any(
        cleaned,
        [r"\binjur", r"\bhurts?\b", r"\bneck pain\b", r"\bhospital\b", r"\burgent care\b", r"\bambulance\b"],
    )
...
def _next_claimant_message(
    ...
    if missing:
        return (
            f"Can you clarify this missing detail so the adjuster can process the claim "
            f"smoothly: {missing[0]}?"
        )
...
def apply_coverage_and_evidence_rules(
    ...
    if classification.claim_type == "home_water_damage":
        coverage_notes.extend(
            [
                "Water damage review usually turns on source of water, suddenness, mitigation, "
                "exclusions, and whether flood coverage is separate.",
                "Do not promise coverage until policy forms, endorsements, cause of loss, and "
                "mitigation facts are reviewed.",
            ]
        )
        if _has_any(
            evidence_text,
            [r"\bunsafe\b", r"\belectrical\b", r"\bsewage\b", r"\bmold\b", r"\bno place to live\b"],
        ):
            add(
                "SAFE-001",
                "urgent",
        ...
        coverage_notes.extend(
            [
                "Auto collision review usually depends on liability facts, coverage type, "
                "deductibles, police report, photos, and damage estimate.",
                "Injury claims require immediate human handling and no medical coverage "
                "promises in the intake response.",
            ]
        )
        ...
        coverage_notes.extend(
            [
                "Theft/property loss review usually requires proof of ownership, loss location, "
                "access details, and a police or incident report.",
        ...
        coverage_notes.extend(
            [
                "Educational/demo note: medical reimbursement review depends on plan terms, "
                "eligibility, dates of service, itemized bills, EOBs, and proof of payment.",
        ...
        coverage_notes.extend(
            [
                "Travel claim review usually depends on covered reason, carrier confirmation, "
                "trip dates, nonrefundable amounts, and refunds or credits received.",
                "Weather-related disruption may require carrier notices and documentation of "
...
def fraud_signal_and_safety_gate(
    ...
    if any(
        item in validation.missing_fields for item in ["date_of_loss", "loss_location", "loss_description"]
    ):
        signal(
            "INTAKE-002",
            "medium",
...
def build_claim_intake_packet(
    ...
    coverage_lines = [f"- {note}" for note in evidence_decision.provisional_coverage_considerations] or [
        "- Coverage review requires policy forms, endorsements, loss facts, and adjuster analysis."
    ]

    signal_lines = [
        f"- `{signal.signal_id}` [{signal.severity}] {signal.message}" for signal in fraud_gate.signals
    ...
    claim_type_label = classification.claim_type.replace("_", " ")
    loss_location_label = claim.loss_location or "an unknown location"
    estimated_loss_label = (
        claim.estimated_loss_usd if claim.estimated_loss_usd is not None else "not supplied"
    )
    handoff = (
        f"{claim.policyholder_name or 'Unknown claimant'} reported a "
        f"{claim_type_label} loss at {loss_location_label} "
        f"on {claim.date_of_loss or 'an unknown date'}. "
        f"Summary: {claim.raw_narrative_summary or claim.loss_description}. "
        f"Estimated loss: {estimated_loss_label}."
    )
